...
class IEMOCAP_6_Ways(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return_dict['input_text'] = Text_Format.format(input=input_text)
        return_dict['input_ids'] = self.text_tokenizer(Text_Format.format(input=input_text), add_special_tokens=True).input_ids
        return_dict['attention_mask'] = [1] * len(return_dict['input_ids'])
        return_dict['input_emotion_diary'] = sample['input_emotion_diary']

        return_dict['responses'] = response
        return_dict['wav_paths'] = audio_paths
        return_dict['response_emotion_diary'] = sample['response_emotion_diary']
        return return_dict
...
class IEMOCAP_DataCollator:
    ...
    def __call__(self, samples: List[Dict]):
        ...
        response = [sample["responses"] for sample in samples]
