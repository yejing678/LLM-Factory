...
def continue_writing(
    ...
    logger.add(f'llama-2-7b-instruct-iemocap-continue-writing.log', format="{time} {level} {message}", level="INFO")
    ...
    def process_dataset(batch):
        batch["input_ids"] = tokenizer(Text_Format.format(input=batch["inputs"])).input_ids
        ...
        batch['input_text'] = Text_Format.format(input=batch["inputs"])
        return batch
